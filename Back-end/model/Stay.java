...
public class Stay implements Serializable {
    ...
    @Id
    @GeneratedValue(strategy = GenerationType.AUTO) // 数据库自动更新id
    private Long id;
    private String name;
    ...
    // mappedBy = "XXX"参数因为它一对多的另一张StayImage那张表上的一的字段名称为"stay"；
    // cascade表示"级联操作"，删除一的时候一对应的多需要一起删除（但是从多对一的那一侧不会做级联删除）
    // fecthType 为 "EAGER"表示读取stay table的时候除了stay table本身的信息之外会把stayImage中的所有信息也读取出来，
    //              "LAZY"表示读取stay数据表的时候不需要读取date的信息（不返回给前端）
